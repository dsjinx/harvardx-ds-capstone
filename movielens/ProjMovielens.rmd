...
2. $\vec{x}_m^{T}$ is a row vector with its elements are all 19 genres we have in the dataset, but with only that movie categorised genres filled with the genre mean, and reset genres slots are all zero. 
3. $\beta_0$ and $\vec\beta_u$ is a set of value to be returned by the fitting, they are in together to represent one single user's genres structure. We used the word structure to stress that, the $\vec{\beta}_u$ has the same length as $\vec{x}_m$, we also assume the user's genres preferences can be categorised in a mixture of these 19 genres we have. And the $\beta_0$ here is to be treated as a central point of all the genres to a user. 
...
#each u_beta$"userId" is a 20x1 vector for the userId = k
#1st element named interception is \beta_0
#the 2nd to 20th elements named as V1 - V19 is the vector \beta_u
...
We will use the the full GD method instead of SGD to find the best $\lambda$, so there is one challenge in computation cost. Since $\lambda$ decides how close our learning result can be to the real minimum point, we want to granulate our increment in the try on sequence. On top of that, we also use the full gradient descent (GD) method instead of SGD to tune the $lambda$ to mimic our final learning computation process. So granular increment try on sequence plus the full size GD together just instantly increase the computation steps exponentially. So to overcome the slow loop obstacle in R, we are using the 'RcppArmadillo' package to write the gradient descent loop in C++, and source the C++ code back to R as a function. We name this C++ sourced function as 'gdtune()' in below code, the C++ code can be found in Appendix at the very end of this report. The manual to use the C++ code can be found on the 'Rcpp' package cran page. 
```{r eval = FALSE}
####lambda
rm(lambda)
...
lmds <- c(seq(0.01, 0.1, length.out = 5),
...
#this C++ loop only returns the 'sum of error^2' for each lambda value
```

```{r eval = FALSE}
#use the sum of error^2 to produce rmse results
lmd_tune <- sqrt(lmd_tune / length(test_R))
```

we can visualise the minimum point in the valley of the $\lambda$ tuning RMSE curve.
```{r echo = FALSE}
...
rm(P, Q, lmds, lmd_tune)##can be kept for plotting
```

Up to now, we have tuned all the important hyperparameters to be ready to give us an expected good result. Rigorously speaking, we have not done all the parameters yet, that is because our training dataset size allow us to risk the epochs (iteration steps) number by guessing. The industrial practice may require the epochs to be tuned as well, because too large epochs may cause the model to be over trained, over training will also increase prediction error like overfitting [@nielsen_2015]. This final piece of code to put all the tuned hyperparameters together is also written in C++, and sourced as function gd(), the C++ code is put in Appendix at very end of report.
```{r eval = FALSE}
...
The goal of prediction \textcolor{blue}{**RMSE:0.8081552**} $<$ \textcolor{red}{0.86490} is reached. 
...
knitr::kable()
```
* "g_mean + u_bias + m_bias + gen_bias + latent_factor" is for demonstration only, the real code in calculation is "g_mean + u_bias + m_bias - gen_bias - latent_factor", please refer the code chunk in above for explanations.  
