...
After we clarified the data structure, then we will proceed to the calculations in below. One thing to notice that, if you are about to run the code, be prepared it can take you about 20 hours, that is the amount of time I got on my windows machine. 
...
$$
We will find the best $\vec{p}_u$ and $\vec{q}_i$ for each user and movie respectively, so that our loss function is minimised. To notice that, instead matrix we used vectors and single values in our loss function, this kind of set up will be more convenient for our sparse matrix case. 
