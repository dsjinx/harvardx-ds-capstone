...
$$
We dropped the u from ui in the $\Sigma$ sign, because we are estimating each user's bias individually based on the all the movies rated by her. We also dropped the N from the first part of $\frac{1}{2N}$ to make our computation formula of $b_u$ to be consistent with the Professor Irizarry's text book, some one may prefer to keep it, that is just personal choice, it will only alter the $\lambda$ result, but the final $b_u$ should not vary much.   
