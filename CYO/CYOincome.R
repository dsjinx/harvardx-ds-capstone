...
#set the foint size for following feature plot with the lattice method
trellis.par.set("fontsize", list(text = 8.5)) 

#boxplot to show all the numeric columns
featurePlot(x = num_fp[, 1:6], y = num_fp$income, plot = "box", 
            scales = list(y = list(relation = "free"),
                          x = list(rot = 90)),
            layout = c(3, 2), auto.key = list(columns = 2))
#scatter plot matrix to show correlations between any two num columns
featurePlot(x = num_fp[, 1:6], y = num_fp$income, plot = "pairs", 
            auto.key = list(columns = 2))

#categorical/character features cols
...
setDF(char_fp) #only dataframe can fit the recursive ggplot in below
#ggmatrix to combine ggplot bars in together
...
fit_forest <- train(income ~., data = train, method = "rf",
                    ...
                    nodesize = best_node,
                    ntree = best_ntree)
varImp(fit_forest, scale = FALSE) #to see what criteria/features used in forest
...
#because we use the data.table meathod to process the data
#all the modifications are all in place changes
#the original "data" will be totally modified
#if want to check the original dataset
#need to go back to load the data again and name it with a different var name
...
#check some extra 0s showing in "education" col
#but presented as in the "10th" string

sapply(data_digit, unique) #browse any other strange values

#convert the char strings into the respective index number
#inside the unique(col.name)
#process the "?" having cols first, force them to be "0"
...
str(data_digit) #check all no missed out strings, every cols are numeric now

#same partition index from trees in above
...
best_cg <- para_grid[which.min(tune_acc), ]#locate the best cost/gamma pair
...
sum_numfull <- data.table(Attributes = c("numeric", "full"), 
                          test_F_1 = c(numgauge_svm$byClass["F1"],
                                       gauge_svm$byClass["F1"]))
knitr::kable(sum_numfull) #compare F_1 with 1st linear svm
...
#the dataset will be modified in place under the data.table
...
modelCor(resamples(ensemble_fit)) #check 3 models correaltionship

#logisctic regression as meta-model for stacking
...
#adaptive boosting as meta-model for stacking
