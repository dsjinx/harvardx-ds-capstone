...
Although there is no "NA", but we can see some "?" showing in "workclass", "occupation", and "native.country", but since they are all in the qualitative attributes, and recorded as character string, so we will decide the way to handle them, when we explore these attributes in details. 
```{r}
sum(is.na(data))
```
```{r echo = FALSE}
...
```
After we normalise all the "integer" columns, it is much easier for us identifying the outliers by inspecting the Min and Max values.  